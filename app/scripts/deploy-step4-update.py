...
manifest = json.loads((source / 'step4-update-manifest.json').read_text())
digest = lambda file: hashlib.sha256(file.read_bytes()).hexdigest()

def checked(base, name):
    relative = Path(name)
    if relative.is_absolute() or '..' in relative.parts or not name.startswith(('app/server/', 'app/dist/client/')):
...
for item in manifest['payload']:
    ...
    if digest(checked(source, item['path'])) != item['sha256']:
        raise SystemExit('Package hash mismatch: ' + item['path'])
backup = Path(tempfile.mkdtemp(prefix='step4-update-backup-', dir='/opt/sheyou-demo-upload'))
for item in manifest['payload']:
...
try:
    ...
    raise
print('Step4 update installed. Log in again; real online A/B/C/D acceptance still required.')
