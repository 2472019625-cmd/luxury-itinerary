...
async function renderActualLayout(data, { root, origin, runId }) {
...
export async function reviewFinalLayout(data, { root, origin, apiKey, baseUrl, model, onProgress = () => {} } = {}) {
  const runId = randomUUID();
  onProgress({ stage: 'final_review', currentAction: '正在生成实际 2000px 长图进行图片复查' });
  const { outputFile, layoutQa, qaFile } = await renderActualLayout(data, { root, origin, runId });
  ...
  if (!apiKey || !placements.some((item) => item.imageSource)) return { runId, outputFile, qaFile, width: preview.width, height: preview.height, failedSlotIds: duplicateSources, warnings: [], modelReviewed: false, outputQa };
  ...
  return { runId, outputFile, qaFile, width: preview.width, height: preview.height, failedSlotIds, reasons: result.reasons || {}, warnings: [...(result.warnings || []), ...lockedWarnings], modelReviewed: true, outputQa };
}
