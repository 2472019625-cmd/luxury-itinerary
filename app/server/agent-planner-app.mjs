...
export function createAgentPlannerServer(options = {}) {
  ...
  const run = async (job, project) => {
    try {
      ...
      store.updateProject(project.projectId, { status: status === "cancelled" ? "cancelled" : "planning_failed", lastError: failure.message });
    }
  };

  const server = createServer(async (request, response) => {
    const url = new URL(request.url, `http://127.0.0.1:${port}`);
    if (request.method === "GET" && url.pathname === "/api/agent/health") return json(response, 200, { ok: true, flowKind: "agent_v1", port, executionEnabled: false, plannerConfigured: Boolean(modelConfig.apiKey) });
    if (request.method === "POST" && url.pathname === "/api/agent/projects") {
      try {
        ...
        const project = store.createProject({ projectId: randomUUID(), flowKind: "agent_v1", executionEnabled: false, status: "planning", activePlanId: null, planIds: [], inputFingerprint, source: { name: String(payload.sourceName || payload.report?.workbookName || "行程资料.xlsx"), sha256: String(payload.sourceSha256 || ""), parser: "deterministic-itinerary-import-v1" }, factBasis, versions: { ruleProfileVersion: AGENT_RULE_PROFILE_VERSION, capabilityConfigVersion: AGENT_CAPABILITY_VERSION, promptVersion: AGENT_PROMPT_VERSION }, createdAt: now, updatedAt: now });
        const job = { jobId: randomUUID(), projectId: project.projectId, status: "planning", message: "正在理解行程", createdAt: now, updatedAt: now, cancelRequested: false };
        jobs.set(job.jobId, job);
        json(response, 202, job);
        setImmediate(() => run(job, project));
    ...
    if (request.method === "POST" && replanMatch) {
      ...
      jobs.set(job.jobId, job); store.updateProject(project.projectId, { status: "planning" }); json(response, 202, job); setImmediate(() => run(job, { ...project, activePlanId: project.activePlanId })); return;
    }
    ...
    if (request.method === "GET" && projectMatch) {
      const active = store.getActive(projectMatch[1]);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  server.listen(port, "127.0.0.1", () => console.log(`智能体试验版｜规划预览：http://127.0.0.1:${port}/agent-planner`));
}
