...
function SimpleManualImagePage({ projectId, ItineraryComponent }) {
  ...
  const workspaceUser = user || { id:"project-designer", name:payload.project.data?.designer?.name || "定制师", profile:payload.project.data?.designer || {}, isAdmin:false };
  ...
  const logout = () => { localStorage.removeItem(AGENT_STORAGE.session); window.location.assign("/agent"); };
