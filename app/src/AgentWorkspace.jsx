...
function Landing() {
  ...
  const upload=async(event)=>{const file=event.target.files?.[0]; if(!file)return; setBusy(true);setError("");try{const [{data,report},sourceSha256]=await Promise.all([importItineraryWorkbook(file,createProductionDefaultData()),digest(file)]);const value=await readJson(await fetch("/api/agent/projects",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({facts:data,report,sourceName:file.name,sourceSha256})}));window.location.assign(`/agent/projects/${value.projectId}`);}catch(e){setError(e.message)}finally{setBusy(false);event.target.value=""}};
  return <div className="agent-shell"><header className="agent-topbar"><div><span className="agent-mark">奢游</span><div><b>行程成品生成智能体</b><small>正式项目入口 · 智能体 V1</small></div></div><div className="agent-mode"><i/>执行能力尚未开放</div></header><section className="agent-hero"><div><p className="agent-eyebrow">AGENT PROJECT WORKSPACE</p><h1>从一份真实行程资料，建立完整的成品项目</h1><p>上传 Excel 后，系统会先检查必须由你确认的关键事实，再制定唯一任务计划。当前阶段只完成项目准备和调度，不会生成文案、联网搜图或导出成品。</p><div className="agent-actions"><button disabled={busy} onClick={()=>input.current?.click()}>{busy?"正在读取资料":"创建智能体项目"}</button><input hidden ref={input} type="file" accept=".xlsx,.xls" onChange={upload}/></div>{error&&<div className="agent-error">{error}</div>}</div><aside><strong>项目边界</strong><ul><li>原始资料始终优先</li><li>关键问题集中确认</li><li>计划与执行状态分离</li><li>下游真实调用保持为零</li></ul></aside></section></div>;
}

export function AgentWorkspace(){const match=window.location.pathname.match(/^\/agent\/projects\/([^/]+)/);return match?<ProjectPage projectId={match[1]}/>:<Landing/>;}
