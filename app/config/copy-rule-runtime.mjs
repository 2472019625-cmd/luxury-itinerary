...
export const COPY_RULE_RUNTIME = Object.freeze([
  ...
  card('COPY-004',['subtitle'],['一句话说明路线气质、核心体验和定制价值'],['酒店清单、照抄、顶级尊享等空泛堆叠'],['路线与核心体验事实'],['具体、有画面且无额外承诺'],['低推理单字段修复，失败升级一次'],['subtitle_specific','subtitle_hotel_list']),
  card('COPY-005',['highlights'],['品牌服务价值与路线独有价值并存；短标题：客户具体价值；保留源强卖点'],['裸标签、口号、换词重复、虚构卖点'],['sourcePosterHighlights与路线事实'],['两类价值存在且路线差异为主'],['单项或模块小范围高推理；仍失败待修订'],['highlight_format','highlight_value','highlight_mix','highlight_duplicate'],'complex'),
  ...
  card('COPY-009',['transportSummary.*.editorialCopy'],['说明场景、类别/等级、已确认承载事实和舒适衔接价值'],['无保证车型承诺、逐卡重复免责声明'],['确定性交通与modelGuaranteed'],['汇总与每日交通对应且承诺一致'],['单项修复；事实/源覆盖问题阻止'],['transport_value','transport_guarantee','transport_no_repeated_disclaimer'],'complex'),
  card('COPY-010',['days.*.description','days.*.spots.*.description'],['写客户做/看什么、现场如何展开、为何值得、如何承接整程；相邻DAY重点不同'],['前往—参观—返回流水账、无依据时段、跨日同义重复、空泛套话'],['当前DAY原始事实、前后DAY摘要、整程DAY角色'],['动作、画面、价值和前后推进均基于事实'],['每次只修一个DAY，复杂问题直接高推理；仍失败待修订'],['day_progression','day_action_scene','day_value','day_near_duplicate','day_no_supplier_log','sentence_fact_evidence'],'complex'),
